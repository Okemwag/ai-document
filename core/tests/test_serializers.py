...
class DocumentUploadSerializerTest(TestCase):
    ...
    def test_valid_txt_file(self):
        file = SimpleUploadedFile(
            "test.txt", b"Sample text content", content_type="text/plain"
        )
        data = {"file": file}
        serializer = DocumentUploadSerializer(data=data)
        self.assertTrue(serializer.is_valid())
    ...
    def test_invalid_file_type(self):
        file = SimpleUploadedFile(
            "test.exe", b"Executable content", content_type="application/x-msdownload"
        )
        data = {"file": file}
        serializer = DocumentUploadSerializer(data=data)
    ...
    def test_invalid_txt_file(self):
        file = SimpleUploadedFile(
            "test.txt", b"\x00\x01\x02", content_type="text/plain"
        )
        data = {"file": file}
        serializer = DocumentUploadSerializer(data=data)
        self.assertFalse(serializer.is_valid())
