...
class DocumentVersionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "version_type", "content", "file", "suggestions", "created_at"]
        read_only_fields = ["id", "created_at"]
        extra_kwargs = {"file": {"required": False}, "suggestions": {"required": False}}
    ...
    def validate_version_type(self, value):
        ...
        if (
            document
            and value == "original"
            and document.versions.filter(version_type="original").exists()
        ):
...
class DocumentImprovementSerializer(serializers.Serializer):
    ...
    improvement_types = serializers.MultipleChoiceField(
        choices=["grammar", "style", "clarity"], required=False
    )
    aggressiveness = serializers.IntegerField(min_value=1, max_value=5, default=3)
        ...
        try:
            ...
            if document.status != "completed":
                raise ValidationError(
                    "Document must be in completed state before export"
                )
            return value
        except Document.DoesNotExist:
